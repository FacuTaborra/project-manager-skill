...
from src.claude_pm.services.search_service import SearchService
...
def test_searches_every_list_in_scope() -> None:
    ...
    matches = SearchService(provider).search_issues("q", project_ids=["a", "b"])
...
def test_no_project_ids_searches_the_whole_workspace() -> None:
    ...
    matches = SearchService(provider).search_issues("q")
...
def test_an_empty_list_of_ids_is_treated_as_global() -> None:
    provider = FakeProvider({None: ["ANY"]})
    assert SearchService(provider).search_issues("q", project_ids=[])
    assert provider.queried == [None]
